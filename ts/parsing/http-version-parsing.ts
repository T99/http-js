...
const REGEX: RegExp = /(?<major>\d+)\.(?<minor>\d+)/u;

export function normalizeHTTPVersion(
	version?: string | number | HTTPVersionObject): HTTPVersionObject {
	
	if (version === undefined || version === null) return DEFAULT_HTTP_VERSION;
	else {
		
		const type: typeof version = typeof version;
		
		if (type === "string") {
		
			const matches: RegExpMatchArray | null =
				REGEX.exec(version as string);
			...
			return normalizeHTTPVersion(version.toString());
